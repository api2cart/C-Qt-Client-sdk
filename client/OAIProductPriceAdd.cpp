/**
 * API2Cart OpenAPI
 * API2Cart
 *
 * The version of the OpenAPI document: 1.1
 * Contact: contact@api2cart.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIProductPriceAdd.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIProductPriceAdd::OAIProductPriceAdd(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIProductPriceAdd::OAIProductPriceAdd() {
    this->initializeModel();
}

OAIProductPriceAdd::~OAIProductPriceAdd() {}

void OAIProductPriceAdd::initializeModel() {

    m_product_id_isSet = false;
    m_product_id_isValid = false;

    m_group_prices_isSet = false;
    m_group_prices_isValid = false;

    m_store_id_isSet = false;
    m_store_id_isValid = false;

    m_idempotency_key_isSet = false;
    m_idempotency_key_isValid = false;
}

void OAIProductPriceAdd::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIProductPriceAdd::fromJsonObject(QJsonObject json) {

    m_product_id_isValid = ::OpenAPI::fromJsonValue(m_product_id, json[QString("product_id")]);
    m_product_id_isSet = !json[QString("product_id")].isNull() && m_product_id_isValid;

    m_group_prices_isValid = ::OpenAPI::fromJsonValue(m_group_prices, json[QString("group_prices")]);
    m_group_prices_isSet = !json[QString("group_prices")].isNull() && m_group_prices_isValid;

    m_store_id_isValid = ::OpenAPI::fromJsonValue(m_store_id, json[QString("store_id")]);
    m_store_id_isSet = !json[QString("store_id")].isNull() && m_store_id_isValid;

    m_idempotency_key_isValid = ::OpenAPI::fromJsonValue(m_idempotency_key, json[QString("idempotency_key")]);
    m_idempotency_key_isSet = !json[QString("idempotency_key")].isNull() && m_idempotency_key_isValid;
}

QString OAIProductPriceAdd::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIProductPriceAdd::asJsonObject() const {
    QJsonObject obj;
    if (m_product_id_isSet) {
        obj.insert(QString("product_id"), ::OpenAPI::toJsonValue(m_product_id));
    }
    if (m_group_prices.size() > 0) {
        obj.insert(QString("group_prices"), ::OpenAPI::toJsonValue(m_group_prices));
    }
    if (m_store_id_isSet) {
        obj.insert(QString("store_id"), ::OpenAPI::toJsonValue(m_store_id));
    }
    if (m_idempotency_key_isSet) {
        obj.insert(QString("idempotency_key"), ::OpenAPI::toJsonValue(m_idempotency_key));
    }
    return obj;
}

QString OAIProductPriceAdd::getProductId() const {
    return m_product_id;
}
void OAIProductPriceAdd::setProductId(const QString &product_id) {
    m_product_id = product_id;
    m_product_id_isSet = true;
}

bool OAIProductPriceAdd::is_product_id_Set() const{
    return m_product_id_isSet;
}

bool OAIProductPriceAdd::is_product_id_Valid() const{
    return m_product_id_isValid;
}

QList<OAIProductAdd_group_prices_inner> OAIProductPriceAdd::getGroupPrices() const {
    return m_group_prices;
}
void OAIProductPriceAdd::setGroupPrices(const QList<OAIProductAdd_group_prices_inner> &group_prices) {
    m_group_prices = group_prices;
    m_group_prices_isSet = true;
}

bool OAIProductPriceAdd::is_group_prices_Set() const{
    return m_group_prices_isSet;
}

bool OAIProductPriceAdd::is_group_prices_Valid() const{
    return m_group_prices_isValid;
}

QString OAIProductPriceAdd::getStoreId() const {
    return m_store_id;
}
void OAIProductPriceAdd::setStoreId(const QString &store_id) {
    m_store_id = store_id;
    m_store_id_isSet = true;
}

bool OAIProductPriceAdd::is_store_id_Set() const{
    return m_store_id_isSet;
}

bool OAIProductPriceAdd::is_store_id_Valid() const{
    return m_store_id_isValid;
}

QString OAIProductPriceAdd::getIdempotencyKey() const {
    return m_idempotency_key;
}
void OAIProductPriceAdd::setIdempotencyKey(const QString &idempotency_key) {
    m_idempotency_key = idempotency_key;
    m_idempotency_key_isSet = true;
}

bool OAIProductPriceAdd::is_idempotency_key_Set() const{
    return m_idempotency_key_isSet;
}

bool OAIProductPriceAdd::is_idempotency_key_Valid() const{
    return m_idempotency_key_isValid;
}

bool OAIProductPriceAdd::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_product_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_group_prices.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_store_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_idempotency_key_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIProductPriceAdd::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
